...
static int goal(float x, float y)
{
    ...
    if (x - BALL_RADIUS > PITCH_X + PITCH_W && ((y - BALL_RADIUS > CENTER_Y - GOAL_HEIGHT / 2 && y + BALL_RADIUS < CENTER_Y + GOAL_HEIGHT / 2) || (y + BALL_RADIUS == CENTER_Y + GOAL_HEIGHT / 2 && y < CENTER_Y + GOAL_HEIGHT / 2) || (y - BALL_RADIUS == CENTER_Y - GOAL_HEIGHT / 2 && y > CENTER_Y - GOAL_HEIGHT / 2)))
    {
        printf("GOAL! Right net hit at x:%.2f, y=%.2f\n", x, y);
        return 1;
    }
    if (x + BALL_RADIUS < PITCH_X && y - BALL_RADIUS > CENTER_Y - GOAL_HEIGHT / 2 && ((y - BALL_RADIUS > CENTER_Y - GOAL_HEIGHT / 2 && y + BALL_RADIUS < CENTER_Y + GOAL_HEIGHT / 2) || (y + BALL_RADIUS == CENTER_Y + GOAL_HEIGHT / 2 && y < CENTER_Y + GOAL_HEIGHT / 2) || (y - BALL_RADIUS == CENTER_Y - GOAL_HEIGHT / 2 && y > CENTER_Y - GOAL_HEIGHT / 2)))
    {
        printf("GOAL! Left net hit at x:%.2f, y=%.2f\n", x, y);
        return 2;
    }
...
static bool out(float x, float y)
{
    ...
    if (x + BALL_RADIUS < PITCH_X || x - BALL_RADIUS > PITCH_X + PITCH_W || y + BALL_RADIUS < PITCH_Y || y - BALL_RADIUS > PITCH_Y + PITCH_H ||
        (y - BALL_RADIUS == CENTER_Y + GOAL_HEIGHT / 2 && y > CENTER_Y + GOAL_HEIGHT / 2) || (y + BALL_RADIUS == CENTER_Y - GOAL_HEIGHT / 2 && y < CENTER_Y - GOAL_HEIGHT / 2))
    {
        printf("Ball is out: x=%.2f, y=%.2f\n", x, y);
        return true;
    }
...
int referee(struct Scene *scene)
{
    ...
    int g = goal(scene->ball->position.x, scene->ball->position.y);
    if (g == 1)
    {
...
void verify_talents(struct Talents talents)
{
    ...
    if (sum > MAX_TALENT_PER_PLAYER || talents.defence > MAX_TALENT_PER_SKILL || talents.agility > MAX_TALENT_PER_SKILL || talents.dribbling > MAX_TALENT_PER_SKILL || talents.shooting > MAX_TALENT_PER_SKILL || talents.defence < 1 || talents.agility < 1 || talents.dribbling < 1 || talents.shooting < 1)
    {
        printf("ERROR: Invalid talents! Values: defence=%d, agility=%d, dribbling=%d, shooting=%d, sum=%d\n",
               talents.defence, talents.agility, talents.dribbling, talents.shooting, sum);
    }
...
void verify_movement(struct Player *player)
{
    ...
    if (player->velocity.x > player->talents.agility * MAX_PLAYER_VELOCITY / MAX_TALENT_PER_SKILL)
    {
        printf(" ERROR: Demanding to run too fast in dimension x! (team %d, player %d)\n", player->team, player->kit);
        player->velocity.x = player->talents.agility * MAX_PLAYER_VELOCITY / MAX_TALENT_PER_SKILL;
    }
    else if (player->velocity.x < -player->talents.agility * MAX_PLAYER_VELOCITY / MAX_TALENT_PER_SKILL)
    {
        printf(" ERROR: Demanding to run too fast in dimension x! (team %d, player %d)\n", player->team, player->kit);
        player->velocity.x = -player->talents.agility * MAX_PLAYER_VELOCITY / MAX_TALENT_PER_SKILL;
    }
    if (player->velocity.y > player->talents.agility * MAX_PLAYER_VELOCITY / MAX_TALENT_PER_SKILL)
    {
        printf(" ERROR: Demanding to run too fast in dimension y! (team %d, player %d)\n", player->team, player->kit);
        player->velocity.y = player->talents.agility * MAX_PLAYER_VELOCITY / MAX_TALENT_PER_SKILL;
    }
    if (player->velocity.y < -player->talents.agility * MAX_PLAYER_VELOCITY / MAX_TALENT_PER_SKILL)
    {
        printf(" ERROR: Demanding to run too fast in dimension y! (team %d, player %d)\n", player->team, player->kit);
...
void verify_shoot(struct Ball *ball, bool kickoff)
{
    ...
    if (ball->velocity.x > ball->possessor->talents.shooting * MAX_BALL_VELOCITY / MAX_TALENT_PER_SKILL)
    {
        printf(" ERROR: Demanding to shoot too fast in dimension x! (team %d, player %d)\n", ball->possessor->team, ball->possessor->kit);
        ball->velocity.x = ball->possessor->talents.shooting * MAX_BALL_VELOCITY / MAX_TALENT_PER_SKILL;
    }
    else if (ball->velocity.x < -ball->possessor->talents.shooting * MAX_BALL_VELOCITY / MAX_TALENT_PER_SKILL)
    {
        printf(" ERROR: Demanding to shoot too fast in dimension x! (team %d, player %d)\n", ball->possessor->team, ball->possessor->kit);
        ball->velocity.x = -ball->possessor->talents.shooting * MAX_BALL_VELOCITY / MAX_TALENT_PER_SKILL;
    }
    if (ball->velocity.y > ball->possessor->talents.shooting * MAX_BALL_VELOCITY / MAX_TALENT_PER_SKILL)
    {
        printf(" ERROR: Demanding to shoot too fast in dimension y! (team %d, player %d)\n", ball->possessor->team, ball->possessor->kit);
        ball->velocity.y = ball->possessor->talents.shooting * MAX_BALL_VELOCITY / MAX_TALENT_PER_SKILL;
    }
    else if (ball->velocity.y < -ball->possessor->talents.shooting * MAX_BALL_VELOCITY / MAX_TALENT_PER_SKILL)
    {
        printf(" ERROR: Demanding to shoot too fast in dimension y! (team %d, player %d)\n", ball->possessor->team, ball->possessor->kit);
        ball->velocity.y = -ball->possessor->talents.shooting * MAX_BALL_VELOCITY / MAX_TALENT_PER_SKILL;
    }
    if (kickoff && ball->position.x == CENTER_X && ball->position.y == CENTER_Y)
    {
        ...
        else if (ball->possessor->team == 2 && ball->velocity.x < 0)
        {
            printf(" ERROR: You must pass to your own half! (team %d, player %d)\n", ball->possessor->team, ball->possessor->kit);
        }
    }
