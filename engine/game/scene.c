...
void update_and_verify_scene_states(struct Scene *scene, const float dt)
{
    ...
    for (int i = 0; i < PLAYER_COUNT; i++)
    {
        ...
        if (p1->position.x > SCREEN_WIDTH - p1->radius)
            p1->position.x = SCREEN_WIDTH - p1->radius;
        if (p1->position.y < p1->radius)
            p1->position.y = p1->radius;
        if (p1->position.y > SCREEN_HEIGHT - p1->radius)
            p1->position.y = SCREEN_HEIGHT - p1->radius;
        if (p2->position.x < p2->radius)
            p2->position.x = p2->radius;
        if (p2->position.x > SCREEN_WIDTH - p2->radius)
            p2->position.x = SCREEN_WIDTH - p2->radius;
        if (p2->position.y < p2->radius)
            p2->position.y = p2->radius;
        if (p2->position.y > SCREEN_HEIGHT - p2->radius)
            p2->position.y = SCREEN_HEIGHT - p2->radius;
    }
